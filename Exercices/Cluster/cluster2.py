...
def main():
    ...
    sample = np.random.randint(70000, size=100)
    ...
    ax.scatter(X_trans[:, 0], X_trans[:, 1], c=cls.labels_,cmap=plt.cm.get_cmap("jet", 10))
    plt.grid('on')

    # figure 2
    ax=fig.add_subplot(122)
    ax.scatter(X_trans[:, 0], X_trans[:, 1], c=db_ytrain, cmap=plt.cm.get_cmap("jet", 10))
    plt.grid('on')
